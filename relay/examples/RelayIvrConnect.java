 ...
 * their choice: 1 - Hear a sales message 2 - Hear a support message 0 - Connect to a live agent at
 * +19184238080
 *
 * <p>The Java RELAY API differs from Python in a few spots used below: - {@code
 * playAndCollect(media, collectConfig, options)} takes three arguments — the trailing {@code
 * options} map is required (pass an empty {@code Map.of()} when you have nothing to add). - {@code
 * connect(devices, options)} takes only a devices matrix and options; there is no ring-back media
 * parameter (play the ring-back TTS separately beforehand). - {@code Call} exposes no {@code
 * waitForEnded()} — poll {@code getState()} / {@code isEnded()} or wait on the specific action's
 * completion future instead.
 *
 ...
 *
 * <p>For full WebSocket / JSON-RPC debug output: SIGNALWIRE_LOG_LEVEL=debug
 */
...
public class RelayIvrConnect {
  ...
  public static void main(String[] args) throws Exception {
    var client = RelayClient.builder().contexts(List.of("default")).build();
        ...
        call -> {
          ...
          var collectAction =
              call.playAndCollect(
                  ...
                  Map.of("digits", Map.of("max", 1, "digit_timeout", 5.0), "initial_timeout", 10.0),
                  Map.of());

          var resultEvent = collectAction.waitForCompletion();
          Map<String, Object> eventParams =
              resultEvent != null ? resultEvent.getParams() : Map.of();
          ...
          if ("digit".equals(resultType) && "1".equals(digits)) {
            ...
            // Support.
            var action =
                call.play(List.of(tts("Please hold while we connect you to our support team.")));
            action.waitForCompletion();
            ...
            var ringBack = call.play(List.of(tts("Connecting you to an agent now. Please hold.")));
            ...
            // No input or invalid.
            var action = call.play(List.of(tts("We didn't receive a valid selection.")));
            action.waitForCompletion();
          }
