 ...
 * com.signalwire.sdk.relay.RelayClient#run()} (it owns the WebSocket and JSON-RPC dispatcher). To
 * place an outbound call without inheriting the lifecycle of an inbound handler, start {@code
 * run()} in a background thread, then issue the dial from the main thread. Hang-up is driven by
 * observing the call's state via {@code getState()} or by waiting on a specific action (such as the
 * TTS playback).
 *
 * <p>Requires env vars: SIGNALWIRE_PROJECT_ID SIGNALWIRE_API_TOKEN SIGNALWIRE_SPACE
 * RELAY_FROM_NUMBER - a number on your SignalWire project RELAY_TO_NUMBER - destination to call
 */
...
public class RelayDialAndPlay {
  ...
  public static void main(String[] args) throws Exception {
    ...
    var playAction =
        call.play(
            List.of(Map.of("type", "tts", "params", Map.of("text", "Welcome to SignalWire"))));
