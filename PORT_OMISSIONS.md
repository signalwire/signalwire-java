...
signalwire.relay.call.Call.wait_for_answered: per-state blocking waiter from Python's legacy Consumer-paradigm Call (relay/call.py); the Java RelayClient-paradigm Call observes state via on(listener) + the typed getCallState() accessor instead of blocking per-state waiters (see RELAY_CLIENT_PARADIGM.md) — same divergence as the already-omitted wait_for/wait_for_ended
signalwire.relay.call.Call.wait_for_ending: per-state blocking waiter from Python's legacy Consumer-paradigm Call; the Java RelayClient-paradigm Call uses on(listener) + getCallState() instead (see Call.wait_for_answered)
signalwire.relay.call.Call.wait_for_ringing: per-state blocking waiter from Python's legacy Consumer-paradigm Call; the Java RelayClient-paradigm Call uses on(listener) + getCallState() instead (see Call.wait_for_answered)
