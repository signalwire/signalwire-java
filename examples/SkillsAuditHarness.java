 ...
 *
 * <ul>
 *   <li>{@code SKILL_NAME} (e.g. {@code web_search}, {@code datasphere})
 *   <li>{@code SKILL_FIXTURE_URL} ({@code http://127.0.0.1:NNNN})
 *   <li>{@code SKILL_HANDLER_ARGS} JSON dict of args for the skill handler
 *   <li>per-skill upstream env (e.g. {@code WEB_SEARCH_BASE_URL}); the audit sets these to point
 *       the skill at its loopback fixture
 *   <li>per-skill credential env vars (e.g. {@code GOOGLE_API_KEY})
 * </ul>
 *
 * <p>For handler-based skills ({@code web_search}, {@code wikipedia_search}, {@code datasphere},
 * {@code spider}) the harness instantiates the skill, registers its tools on a temporary {@code
 * Service}, and dispatches the documented tool name with the parsed args. The skill's handler
 * issues real HTTP through {@code java.net.http.HttpClient} (proven by the audit's fixture seeing
 * the request).
 *
 * <p>For DataMap-based skills ({@code api_ninjas_trivia}, {@code weather_api}) the SignalWire
 * platform -- not the SDK -- would normally fetch the configured webhook URL. The harness simulates
 * that platform behavior by extracting the webhook URL from the registered DataMap and issuing the
 * HTTP call itself, satisfying the audit's contract that "the SDK contacted the upstream" via real
 * bytes on the wire.
 *
 * <p>Stdout: parsed return value as JSON. Exits 0 on success, non-zero on any failure.
 */
...
public class SkillsAuditHarness {
  ...
  public static void main(String[] args) {
    ...
    Object result =
        switch (skillName) {
          ...
          case "api_ninjas_trivia" ->
              executeDataMap(service, "get_trivia", ensureCategory(handlerArgs));
  ...
  /**
   * Dispatch a handler-based tool through the Service's SWAIG dispatcher. The handler issues a real
   * HTTP request to the configured upstream (the audit fixture).
   */
  private static Map<String, Object> dispatchHandler(
      Service service, String toolName, Map<String, Object> args) {
    Map<String, Object> rawData =
        Map.of("call_id", "audit-call", "global_data", Collections.emptyMap());
    FunctionResult fr = service.onFunctionCall(toolName, args, rawData);
    if (fr == null) {
      return Map.of("error", "tool '" + toolName + "' not registered or returned null");
    }
    return fr.toMap();
  }

  /**
   * Extract the webhook URL from the named DataMap tool and execute it ourselves. This is what the
   * SignalWire platform does in production; the audit just verifies the URL shape and the SDK's
   * parsing.
   */
  ...
  private static Map<String, Object> executeDataMap(
    ...
    try {
      ...
      return Map.of("error", "HTTP " + method + " " + url + " failed: " + e.getMessage());
    }
  }

  /**
   * Naive {@code ${args.field}} / {@code %{args.field}} template expansion for DataMap webhook
   * URLs. The Java SDK's DataMap webhook URLs use {@code ${args.X}} (matching SWML's
   * variable-reference syntax). Both forms are accepted so the harness works against any port that
   * emits either dialect. Recognized prefixes:
   *
   * <ul>
   *   <li>{@code args.X} -- raw value
   *   <li>{@code enc:args.X} -- URL-encoded
   *   <li>{@code lc:enc:args.X} -- lowercased + URL-encoded
   * </ul>
   *
   * Unknown markers are left in place so the audit fixture sees what the SDK would emit on the
   * wire.
   */
