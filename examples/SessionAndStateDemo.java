...
public class SessionAndStateDemo {
  ...
  public static void main(String[] args) throws Exception {
    var agent = AgentBase.builder().name("stateful-agent").route("/").port(3000).build();

    agent.promptAddSection(
        "Role", "You are a shopping assistant. Help users add items to their cart.");
    agent.promptAddSection(
        "Instructions",
        "",
        List.of(
    ...
    agent.updateGlobalData(Map.of("cart", Map.of("items", List.of(), "total", 0)));
    ...
    agent.defineTool(
        ...
        (toolArgs, raw) -> {
          ...
          String lastItem =
              globalData != null ? (String) globalData.getOrDefault("last_item", "none") : "none";
    ...
    agent.defineTool(
        ...
        Map.of("type", "object", "properties", Map.of()),
        (toolArgs, raw) ->
            new FunctionResult("Order placed successfully! Thank you for shopping with us.")
                .hangup());
