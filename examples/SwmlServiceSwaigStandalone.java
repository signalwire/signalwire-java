 ...
 * {@code /swaig} endpoint.
 *
 * <p>This is the path you take when you want a SWAIG-callable HTTP service that isn't an {@code
 * <ai>} agent: the SWAIG verb is a generic LLM-tool surface and {@code Service} is the host. {@code
 * AgentBase} is a {@code Service} subclass that <em>also</em> layers in prompts, AI config, dynamic
 * config, and token validation -- but {@code defineTool}, {@code registerSwaigFunction}, and {@code
 * onFunctionCall} all live on {@code Service} itself.
 *
 * <p>The class extends {@link com.signalwire.sdk.swml.Service} directly and registers its SWAIG
 * tools in the constructor. {@code main()} only calls {@code serve()}. This shape is reusable: the
 * SDK's {@code swaig-test} CLI can load the class in-process via reflection and inspect the tool
 * registry without starting an HTTP server.
 *
 * <p>Run as a server:
 *
 * <pre>
