 ...
 * to customize its behavior per-request (e.g., multi-tenancy).
 */
...
public class SimpleDynamicAgent {
  ...
  public static void main(String[] args) throws Exception {
    var agent = AgentBase.builder().name("dynamic-agent").route("/").port(3000).build();

    agent.promptAddSection("Role", "You are a customizable assistant.");
    ...
    agent.setDynamicConfigCallback(
        (queryParams, bodyParams, headers, configAgent) -> {
          ...
          configAgent.promptAddSection("Tenant", "You are serving tenant: " + tenant);
          ...
          if (!customHeader.isEmpty()) {
            configAgent.promptAddSection("Custom", "Custom config: " + customHeader.get(0));
          }
        });
