 ...
 *
 * <p>The {@code ai_sidecar} verb runs an AI listener alongside an in-progress call (real-time
 * copilot, transcription analyzer, compliance monitor, etc.). It is NOT an agent -- it does not own
 * the call. So the right host is {@code Service}, not {@code AgentBase}.
 *
 * <p>The class extends {@link com.signalwire.sdk.swml.Service} directly so the SDK's {@code
 * swaig-test} CLI can load it in-process via reflection (no HTTP) and inspect its tool registry --
 * the SWML doc + tools live in the constructor, {@code main()} only calls {@code serve()}.
 *
 * <p>What this serves:
 *
 * <pre>
...
public class SwmlServiceAiSidecar extends Service {
  ...
  /**
   * Public URL of this service (env-overridable). The ai_sidecar verb needs a fully-qualified URL
   * to POST SWAIG calls back to. Reading the env at construction time keeps the SWML doc and the
   * tool registry build-once-and-reusable.
   */
  private static String resolvePublicUrl() {
    return System.getenv()
        .getOrDefault("PUBLIC_URL", "https://your-host.example.com/sales-sidecar");
  }
  ...
  public SwmlServiceAiSidecar() {
    ...
    addVerbToSection(
        ...
        Map.of(
            ...
            "direction",
            List.of("remote-caller", "local-caller"),
            ...
            "SWAIG",
            Map.of("defaults", Map.of("web_hook_url", publicUrl + "/swaig"))));
    hangup();
