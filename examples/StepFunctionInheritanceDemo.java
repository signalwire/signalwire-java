 ...
 * INHERITS from the previous step when omitted.
 *
 * <h2>Why this matters</h2>
 *
 * A common mistake when building multi-step agents is to assume each step starts with a fresh tool
 * set. It does not. The runtime only resets the active set when a step explicitly declares its
 * {@code functions} field. If you forget {@code setFunctions()} on a later step, the previous
 * step's tools quietly remain available.
 *
 * <p>This file shows four step-shaped patterns side by side:
 *
 * <ol>
 *   <li>{@code step_lookup} — explicitly whitelists {@code lookup_account}
 *   <li>{@code step_inherit} — has NO {@code setFunctions()} call. Inherits step_lookup's
 *       whitelist, so {@code lookup_account} is still callable here. This is rarely what you want.
 *   <li>{@code step_explicit} — explicitly whitelists {@code process_payment}. The previously
 *       inherited {@code lookup_account} is now disabled, and only {@code process_payment} is
 *       active.
 *   <li>{@code step_disabled} — explicitly disables ALL user functions with an empty list (or
 *       {@code "none"}). Internal tools like {@code next_step} still work.
 * </ol>
 *
 * <h2>Best practice</h2>
 *
 * Call {@code setFunctions()} on EVERY step that should differ from the previous one. Treat
 * omission as an explicit decision to inherit, not a default.
 *
 * <p>Run this file to see the rendered SWML — there are no real webhook endpoints behind the tools,
 * this is purely a documentation example.
 */
...
public class StepFunctionInheritanceDemo {
  ...
  public static void main(String[] args) throws Exception {
    var agent =
        AgentBase.builder().name("step_function_inheritance_demo").route("/").port(3000).build();
    ...
    agent.defineTool(
        ...
        Map.of("type", "object", "properties", Map.of("account_number", Map.of("type", "string"))),
        (a, raw) -> new FunctionResult("looked up"));

    agent.defineTool(
        "process_payment",
        "Process a payment for the current customer",
        Map.of("type", "object", "properties", Map.of("amount", Map.of("type", "number"))),
        (a, raw) -> new FunctionResult("payment processed"));

    agent.defineTool(
        "send_receipt",
        "Email a receipt to the customer",
        Map.of("type", "object", "properties", Map.of("email", Map.of("type", "string"))),
