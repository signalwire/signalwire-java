 ...
 * fallback outputs -- all without custom webhook endpoints.
 */
...
public class AdvancedDatamapDemo {
  ...
  public static void main(String[] args) throws Exception {
    var agent = AgentBase.builder().name("advanced-datamap").route("/").port(3000).build();

    agent.promptAddSection(
        "Role", "You are an assistant that demonstrates advanced DataMap features.");

    // 1. Expression-based command processor with pattern matching
    var commandMap =
        new DataMap("command_processor")
            ...
            .expression(
                "${args.command}", "^start", new FunctionResult("Starting process: ${args.target}"))
            .expression(
                "${args.command}", "^stop", new FunctionResult("Stopping process: ${args.target}"))
            .expression(
                "${args.command}",
                "^status",
    ...
    var searchMap =
        new DataMap("search_results")
            ...
            .output(
                new FunctionResult("Search results for \"${args.query}\":\n\n${formatted_results}"))
    ...
    var calcMap =
        new DataMap("smart_calculator")
            ...
            .expression(
                ...
                "^detailed$",
                new FunctionResult(
                    "Detailed: ${args.expression} = @{expr ${args.expression}} at @{strftime_tz UTC %Y-%m-%d %H:%M:%S}"))
            .fallbackOutput(
                new FunctionResult(
