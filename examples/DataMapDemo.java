 ...
 * using variable expansion and pattern matching.
 */
...
public class DataMapDemo {
  ...
  public static void main(String[] args) throws Exception {
    var agent = AgentBase.builder().name("datamap-agent").route("/").port(3000).build();

    agent.promptAddSection(
        "Role", "You are an assistant that can look up weather and check order status.");
    ...
    var statusMap =
        new DataMap("check_order_status")
            ...
            .expression(
                ...
                new FunctionResult(
                    "Invalid order number format. Orders start with ORD- followed by digits."));
