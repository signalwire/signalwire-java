 ...
 * skill. The gateway bridges MCP tools so the agent can use them as SWAIG functions.
 *
 * <p>Prerequisites: Start an MCP gateway server: mcp-gateway -c config.json
 *
 * <p>Env vars: MCP_GATEWAY_URL - URL of the running gateway (default: http://localhost:8080)
 * MCP_GATEWAY_AUTH_USER - Basic auth username MCP_GATEWAY_AUTH_PASSWORD - Basic auth password
 */
...
public class McpGatewayDemo {
  ...
  public static void main(String[] args) throws Exception {
    var agent =
        AgentBase.builder().name("mcp-gateway-agent").route("/mcp-gateway").port(3000).build();
