 ...
 *
 * <p>Mirror of Python's {@code examples/basic_swml_service.py}. Shows four canonical "no AI"
 * call-control flows: voicemail, IVR menu, call transfer, and call recording. Pick one with the
 * first CLI arg ({@code voicemail}, {@code ivr}, {@code transfer}, or {@code record}) -- defaults
 * to {@code voicemail}.
 *
 * <pre>
...
public class BasicSwmlService {
  ...
  public static void main(String[] args) throws Exception {
    ...
    switch (mode) {
      ...
      default -> {
        System.out.println("Usage: BasicSwmlService [voicemail|ivr|transfer|record]");
        System.exit(1);
      }
    }
  }
  ...
  static void startVoicemail() throws Exception {
    ...
    svc.record(
        Map.of(
            "format", "mp3", "stereo", false, "beep", true, "max_length", 120, "terminators", "#"));
  ...
  static void startIvr() throws Exception {
    ...
    svc.prompt(
        Map.of(
            "play",
            "say:Welcome. Press 1 for sales, 2 for support, " + "or 3 to leave a message.",
            "max_digits",
            1,
            "terminators",
            "#",
  ...
  static void startTransfer() throws Exception {
    ...
    svc.connect(Map.of("from", "+15551234567", "timeout", 30, "answer_on_bridge", true));
    svc.play(Map.of("url", "say:All agents are busy. Please leave a message."));
    svc.record(
        Map.of(
            "format", "mp3",
            "max_length", 120,
  ...
  static void startRecording() throws Exception {
    ...
    svc.play(
        Map.of(
            "url", "say:This call is being recorded. " + "Please tell us about your experience."));
    svc.sleep(30000);
