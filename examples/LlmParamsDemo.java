 ...
 * different response characteristics: - Precise (low temperature, hard to interrupt) - Creative
 * (high temperature, easy to interrupt) - Customer Service (balanced)
 *
 * <p>Usage: java LlmParams [precise|creative|support]
 */
...
public class LlmParamsDemo {
  ...
  static void startPrecise() throws Exception {
    var agent = AgentBase.builder().name("precise-assistant").route("/precise").port(3000).build();
  ...
  static void startCreative() throws Exception {
    var agent =
        AgentBase.builder().name("creative-assistant").route("/creative").port(3000).build();
    ...
    agent.promptAddSection(
        ...
        "",
        List.of(
            "Be imaginative and creative", "Use varied vocabulary", "Encourage creative thinking"));
    ...
    agent.defineTool(
        ...
        Map.of(
            ...
            "object",
            "properties",
            Map.of("theme", Map.of("type", "string", "description", "Story theme"))),
        (toolArgs, raw) -> {
          String theme = (String) toolArgs.getOrDefault("theme", "adventure");
          return new FunctionResult(
              "Story prompt for " + theme + ": A map that only appears during thunderstorms");
        });
  ...
  static void startSupport() throws Exception {
    var agent = AgentBase.builder().name("customer-service").route("/support").port(3000).build();
    ...
    agent.defineTool(
        ...
        Map.of(
            ...
            "object",
            "properties",
            Map.of("order_id", Map.of("type", "string", "description", "Order ID")),
            "required",
            List.of("order_id")),
        (toolArgs, raw) -> {
          String orderId = (String) toolArgs.getOrDefault("order_id", "unknown");
          return new FunctionResult(
              "Order " + orderId + " status: Shipped - expected delivery tomorrow.");
        });
