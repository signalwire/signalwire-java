 ...
 * questions, ALL of the step's other functions are forcibly deactivated. The only callable tools
 * during a gather question are:
 *
 * <ul>
 *   <li>{@code gather_submit} (the native answer-submission tool, always active)
 *   <li>Whatever names you list in that question's {@code functions} argument
 * </ul>
 *
 * <p>{@code next_step} and {@code change_context} are also filtered out — the model literally
 * cannot navigate away until the gather completes. This is by design: it forces a tight ask →
 * submit → next-question loop.
 *
 * <p>If a question needs to call out to a tool — for example, to validate an email format, geocode
 * a ZIP, or look up something from an external service — you must list that tool name in the
 * question's {@code functions} argument. The function is active ONLY for that question.
 *
 * <p>Below: a customer-onboarding gather flow where each question unlocks a different validation
 * tool, and where the step's own non-gather tools ({@code escalate_to_human}, {@code
 * lookup_existing_account}) are LOCKED OUT during gather because they aren't whitelisted on any
 * question.
 *
...
public class GatherPerQuestionFunctionsDemo {
  ...
  public static void main(String[] args) throws Exception {
    ...
    agent.defineTool(
        ...
        Map.of("type", "object", "properties", Map.of("zip", Map.of("type", "string"))),
        (a, raw) -> new FunctionResult("{\"city\":\"...\",\"state\":\"...\"}"));

    agent.defineTool(
        "check_age_eligibility",
        "Verify the customer is old enough for the product",
        Map.of("type", "object", "properties", Map.of("age", Map.of("type", "integer"))),
        ...
        ctx.addStep("onboard")
            ...
            .setFunctions(
                List.of(
                    ...
                    "escalate_to_human", "lookup_existing_account"))
            .setGatherInfo(
                "customer",
                "next_step",
