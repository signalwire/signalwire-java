...
signalwire.rest.namespaces.calling_resources_generated.Calling.get_base_path: Java's generated command-dispatch Calling class exposes getBasePath() as a public plumbing accessor (same idiom as CrudResource.getBasePath()) so the RouteRegistry/mock tests can read the composed /calling/calls base path; Python's command-dispatch resource keeps the base path private
signalwire.rest._base.BaseResource.get_base_path: Java's hand BaseResource (root of the generated REST resource hierarchy) exposes getBasePath() as a public plumbing accessor; Python's _base.BaseResource keeps the path private
signalwire.rest._base.BaseResource.get_http_client: Java's hand BaseResource exposes getHttpClient() so subclasses/tests can reach the shared transport; Python's _base.BaseResource keeps the client private
signalwire.rest._base.CrudResource.get: Java's CrudResource keeps a public get(id) alongside create/update/delete (existing REST-COVERAGE + namespace tests call .get(id)); Python routes reads through ReadResource, so its CrudResource carries no get
signalwire.rest._base.CrudResource.list: Java's CrudResource keeps a public list()/list(Map) alongside create/update/delete (existing REST-COVERAGE + namespace tests call .list()); Python routes reads through ReadResource, so its CrudResource carries no list
signalwire.rest._base.FabricResource.__init__: idiomatic Java surface extension (constructor) not present in Python's parameterless _base.FabricResource
signalwire.rest._base.FabricResource.list_addresses: Java folds Python's CrudWithAddresses.list_addresses onto FabricResource (the base for all Fabric CRUD resources), matching every other port (Go's CrudWithAddresses.ListAddresses). The list_addresses route is spec-defined for every fabric resource and present in the reference's runtime route registry; python_signatures.json records it on CrudWithAddresses, not the empty _base.FabricResource — an oracle blind spot (L12). Same GET {base}/{id}/addresses wire call.
signalwire.rest._base.FabricResourcePUT.__init__: idiomatic Java surface extension (constructor) not present in Python's parameterless _base.FabricResourcePUT
signalwire.rest._base.ReadResource.__init__: idiomatic Java surface extension (constructor) not present in Python's parameterless _base.ReadResource
