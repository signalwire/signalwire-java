 ...
 * (playTts/playAudio/playSilence/playRingtone, detectDigit/ detectAnsweringMachine/detectFax,
 * promptTts/promptAudio).
 *
 * <p>Each test dials a Call against the shared mock relay, invokes a convenience method, then
 * asserts the EXACT RELAY media/detect wire shape that landed in the mock's receive journal. These
 * are behavioral + wire assertions against the real shared mock — never a transport mock. Mirrors
 * the Python reference convenience wrappers in {@code signalwire/relay/call.py}.
 */
class CallConvenienceMockTest {
  ...
  private Map<String, Object> lastPromptParams() {
    List<RelayMockTest.JournalEntry> pc = mock.journalRecv(Constants.METHOD_PLAY_AND_COLLECT);
  ...
  void detectAmdMinimalShape() {
    ...
    assertTrue(paramsOf(detect).isEmpty(), "machine params must contain only the supplied keys");
  }
