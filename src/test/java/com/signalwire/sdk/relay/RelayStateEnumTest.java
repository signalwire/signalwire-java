...
/**
 * Pure-unit tests (no mocks, no transport) for the Tier-3 typed RELAY state enums ({@link
 * CallState} / {@link DialState} / {@link MessageState}) and the {@link Device} struct.
 *
 * <p>What these pin, against real directly-constructed / directly-dispatched objects:
 *
 * <ul>
 *   <li>{@code fromWire} round-trips every constant's {@code getValue()} and returns {@code null}
 *       (never throws) for an unknown / null string;
 *   <li>{@code isTerminal()} matches the reference's terminal sets exactly (CallState: only ENDED;
 *       DialState: ANSWERED+FAILED; MessageState: DELIVERED+UNDELIVERED+FAILED — NOT RECEIVED) and
 *       agrees with the existing {@code Constants.isTerminal*} string predicates;
 *   <li>the typed accessors ({@code Call.getCallState()} / {@code Message.getMessageState()} /
 *       {@code CallDialEvent.getDialStateEnum()}) agree with the parity string getter on a real
 *       dispatched event;
 *   <li>the three vocabularies are never conflated (a CallState value is not a MessageState value,
 *       etc.);
 *   <li>{@code Device.toMap()} is byte-identical to the hand-written {@code {type, params}} map
...
class RelayStateEnumTest {
  ...
  class CallStateTests {
    ...
    void fromWireRoundTrips() {
      for (CallState s : CallState.values()) {
        assertSame(s, CallState.fromWire(s.getValue()), "round-trip failed for " + s);
      }
    }
    ...
    void isTerminalAgreesWithStringPredicate() {
      for (CallState s : CallState.values()) {
        assertEquals(
            Constants.isTerminalCallState(s.getValue()),
            s.isTerminal(),
  ...
  class DialStateTests {
    ...
    void fromWireRoundTrips() {
      for (DialState s : DialState.values()) {
        assertSame(s, DialState.fromWire(s.getValue()), "round-trip failed for " + s);
      }
    }
  ...
  class MessageStateTests {
    ...
    void fromWireRoundTrips() {
      for (MessageState s : MessageState.values()) {
        assertSame(s, MessageState.fromWire(s.getValue()), "round-trip failed for " + s);
      }
    }
    ...
    void isTerminalAgreesWithStringPredicate() {
      for (MessageState s : MessageState.values()) {
        assertEquals(
            Constants.isTerminalMessageState(s.getValue()),
            s.isTerminal(),
            "enum/string terminal disagreement for " + s);
      }
    }
  ...
  @Test
  @DisplayName(
      "Call.getCallState() is empty for an unknown raw state (getState() still returns it)")
  ...
  void dialEventTypedAccessorAgreesWithStringGetter() {
    for (DialState expected : DialState.values()) {
      ...
      RelayEvent.CallDialEvent event =
          new RelayEvent.CallDialEvent(Constants.EVENT_CALL_DIAL, 0.0, inner);
  ...
  void deviceToMapMatchesHandWritten() {
    ...
    assertEquals(List.copyOf(handWritten.keySet()), List.copyOf(fromDevice.keySet()));
    @SuppressWarnings("unchecked")
    Map<String, Object> fromParams = (Map<String, Object>) fromDevice.get("params");
    assertEquals(List.copyOf(handParams.keySet()), List.copyOf(fromParams.keySet()));
  }
  ...
  void deviceWireShapeParityForDial() {
    ...
    List<List<Map<String, Object>>> typedDevices = List.of(List.of(typed.toMap()));
  ...
  void deviceNullHandlingAndDefensiveCopy() {
    ...
    assertEquals(
        ...
        d.toMap().get("params") instanceof Map
            ? ((Map<?, ?>) d.toMap().get("params")).get("to")
            : null);
  }
  ...
  void deviceSipFactoryOmitsNullFrom() {
    ...
    assertFalse(
        noFrom.getParams().containsKey("from"), "null from must be omitted, not stored as null");
  }
}
