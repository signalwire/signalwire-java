...
/**
 * Mock-backed unit tests translated from signalwire-python/tests/unit/rest/test_calling_mock.py.
 *
 * <p>Every command in {@link com.signalwire.sdk.rest.namespaces.CallingNamespace} is exercised
 * against the mock server so we know the SDK sends the right wire request — method, path, command
 * field, id, and params.
 */
class CallingMockTest {
  ...
  /**
   * Asserts the journal entry shape — method/path/command — and returns the params map for
   * caller-specific assertions. {@code expectedId} may be null to require no {@code id} field at
   * the body root (true for dial / update which carry id inside params).
   */
  private Map<String, Object> commandAssert(
      MockTest.JournalEntry j, String command, String expectedId) {
    ...
    if (expectedId == null) {
      assertFalse(body.containsKey("id"), "expected no id at body root, got " + body.get("id"));
    } else {
  ...
  class Collect {
    ...
    void collectStartInputTimers() {
      ...
      assertTrue(body.containsKey("id"));
      Map<String, Object> p =
          commandAssert(mock.last(), "calling.collect.start_input_timers", "call-1");
      assertEquals("col-1", p.get("control_id"));
    }
  }
