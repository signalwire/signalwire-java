 ...
 *
 * <p>Each Java test mirrors one Python test and asserts on both the SDK response body shape and the
 * wire request the mock journaled.
 */
class CompatCallsStreamsMockTest {
  ...
  class StartStream {
    ...
    void returnsStreamResource() {
      ...
      assertTrue(
          result.containsKey("sid") || result.containsKey("name"),
    ...
    void journalRecordsPostToStreamsCollection() {
      ...
      assertEquals(acctBase + "/Calls/CA_JX1/Streams", j.path, "path");
  ...
  class StopStream {
    ...
    void returnsStreamResourceWithStatus() {
      ...
      Map<String, Object> result = client.compat().calls().stopStream("CA_T1", "ST_T1", body);
      assertNotNull(result, "stopStream returned null");
      // The stop endpoint synthesises a stream resource (sid + status).
      assertTrue(
          result.containsKey("sid") || result.containsKey("status"),
    ...
    void journalRecordsPostToSpecificStream() {
      ...
      MockTest.JournalEntry j = mock.last();
      assertEquals("POST", j.method, "method");
      assertEquals(acctBase + "/Calls/CA_S1/Streams/ST_S1", j.path, "path");
      Map<String, Object> jb = j.bodyMap();
      assertNotNull(jb);
  ...
  class UpdateRecording {
    ...
    void returnsRecordingResource() {
      ...
      Map<String, Object> result = client.compat().calls().updateRecording("CA_T2", "RE_T2", body);
    ...
    void journalRecordsPostToSpecificRecording() {
      ...
      MockTest.JournalEntry j = mock.last();
      assertEquals("POST", j.method, "method");
      assertEquals(acctBase + "/Calls/CA_R1/Recordings/RE_R1", j.path, "path");
      Map<String, Object> jb = j.bodyMap();
      assertNotNull(jb);
