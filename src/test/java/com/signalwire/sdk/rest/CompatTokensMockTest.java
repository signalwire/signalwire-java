...
/**
 * Mock-backed unit tests translated from signalwire-python/tests/unit/rest/test_compat_tokens.py.
 *
 * <p>{@code CompatTokens.update} uses PATCH (not POST) — it inherits the {@code BaseResource}
 * convention rather than the Twilio-style POST.
 */
class CompatTokensMockTest {
  ...
  class Create {
    ...
    void returnsToken() {
      Map<String, Object> result = client.compat().tokens().create(kw("Ttl", 3600));
      assertNotNull(result);
      assertTrue(
          result.containsKey("token") || result.containsKey("id"),
  ...
  class Update {
    ...
    @DisplayName("returns_token_resource")
    void returnsToken() {
      Map<String, Object> result = client.compat().tokens().update("TK_U", kw("Ttl", 7200));
      assertNotNull(result);
