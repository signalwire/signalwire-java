...
/**
 * Mock-backed unit tests translated from signalwire-python/tests/unit/rest/test_compat_accounts.py.
 *
 * <p>Drives {@code client.compat().accounts()} against the live mock server and asserts both the
 * SDK return value and the recorded request journal.
 */
class CompatAccountsMockTest {
  ...
  class Create {
    ...
    void returnsAccountResource() {
      Map<String, Object> result = client.compat().accounts().create(kw("FriendlyName", "Sub-A"));
      assertNotNull(result);
      assertTrue(
          result.containsKey("friendly_name"), "expected friendly_name, got " + result.keySet());
    }
  ...
  class Get {
    ...
    void journalRecordsGet() {
      ...
      assertTrue(
          j.body == null
              || (j.body instanceof Map && ((Map<?, ?>) j.body).isEmpty())
