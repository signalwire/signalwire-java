 ...
 *
 * <ul>
 *   <li>Path routing (root route, non-root route, health, 404)
 *   <li>Basic auth (including the 401 path for invalid credentials)
 *   <li>SWAIG dispatch with parsed args
 *   <li>Post-prompt summary callback invocation
 *   <li>SWML rendering with Lambda-derived webhook URLs
 *   <li><b>The route-preservation regression test</b> — non-root route + Lambda env vars + {@code
 *       SWML_PROXY_URL_BASE} must still emit webhook URLs that include the agent's route before
 *       {@code /swaig}.
 * </ul>
 */
class LambdaAgentHandlerTest {
  ...
  private static Map<String, Object> v2Event(
  ...
  private static AgentBase buildAgent(String route) {
    ...
    agent.defineTool(
        ...
        (args, raw) -> new FunctionResult("Hello, " + args.getOrDefault("name", "friend") + "!"));
    return agent;
  }
  ...
  void postPromptInvokesSummaryCallback() {
    ...
    Map<String, String> headers = Map.of("Authorization", basicAuth("u", "p"));
    String payload =
        GSON.toJson(Map.of("post_prompt_data", Map.of("parsed", Map.of("name", "Test"))));
   ...
   *
   * <p>An agent mounted at a non-root route MUST have its route preserved in rendered webhook URLs,
   * even when a proxy base URL and Lambda env vars are both present. The proxy short-circuit must
   * NOT return the bare proxy URL without the agent's route.
   */
  @Test
  @SuppressWarnings("unchecked")
  ...
  void routePreservedWithSynthesisedLambdaUrl() {
    ...
    assertTrue(
        swaigUrl.contains("/widgets/swaig"),
