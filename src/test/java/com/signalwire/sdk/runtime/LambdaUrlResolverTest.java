...
class LambdaUrlResolverTest {
  ...
  void functionUrlTakesPrecedence() {
    ...
    assertEquals(
        "https://abc.lambda-url.us-west-2.on.aws", new LambdaUrlResolver(env).resolveBaseUrl());
  }

  @Test
  void trailingSlashStrippedFromFunctionUrl() {
    var env = envOf(Map.of("AWS_LAMBDA_FUNCTION_URL", "https://abc.lambda-url.us-west-2.on.aws/"));
    assertEquals(
        "https://abc.lambda-url.us-west-2.on.aws", new LambdaUrlResolver(env).resolveBaseUrl());
  }
  ...
  void synthesisedFromNameAndRegion() {
    ...
    assertEquals(
        "https://my-agent.lambda-url.eu-west-1.on.aws",
  ...
  void synthesisedUsesDefaultRegionWhenMissing() {
    ...
    assertEquals(
        "https://my-agent.lambda-url.us-east-1.on.aws",
        new LambdaUrlResolver(env).resolveBaseUrl());
  }
