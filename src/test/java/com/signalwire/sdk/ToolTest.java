...
class ToolTest {
  ...
  @BeforeEach
  void setUp() {
    agent = AgentBase.builder().name("tool-test").authUser("u").authPassword("p").build();
  }
  ...
  void testDefineToolByDefinition() {
    ToolDefinition td =
        new ToolDefinition(
            "calc", "Calculate", Map.of("type", "object"), (a, r) -> new FunctionResult("42"));
    agent.defineTool(td);
  ...
  void testToolWithMultipleArgs() {
    ...
    FunctionResult result =
        agent.onFunctionCall("concat", Map.of("a", "foo", "b", "bar"), Map.of());
  ...
  void testToolDefinitionProperties() {
    ToolDefinition td =
        new ToolDefinition(
            "name", "desc", Map.of("type", "object"), (a, r) -> new FunctionResult("ok"));
  ...
  void testToolDefinitionToSwaigFunctionWithToken() {
    ToolDefinition td =
        new ToolDefinition("secure_tool", "Secure", Map.of(), (a, r) -> new FunctionResult("ok"));
  ...
  void testToolDefinitionExtraFieldsMergedIntoSwaig() {
    ToolDefinition td =
        new ToolDefinition("tool", "desc", Map.of(), (a, r) -> new FunctionResult("ok"));
