...
/**
 * Proves the typed {@link RecordFormat} enum and the equivalent bare string produce the IDENTICAL
 * record-call payload — both through the SWAIG {@link FunctionResult#recordCall} action and through
 * the AgentBase builder's {@code recordFormat} setter. No mocks: real FunctionResult / real
 * AgentBase rendering the real SWML.
 */
class RecordFormatTest {
  ...
  void functionResultRecordCallEnumEqualsString() {
    ...
    FunctionResult viaEnum =
        new FunctionResult("Recording").recordCall("ctrl-1", true, RecordFormat.MP3, "both");
    FunctionResult viaString =
        new FunctionResult("Recording").recordCall("ctrl-1", true, "mp3", "both");
  ...
  void functionResultNullFormatEnumMatchesStringDefault() {
    ...
    FunctionResult viaEnum =
        new FunctionResult("Recording").recordCall(null, false, (RecordFormat) null, "both");
    FunctionResult viaString =
        new FunctionResult("Recording").recordCall(null, false, (String) null, "both");
    assertEquals(viaString.toMap(), viaEnum.toMap());
  }
