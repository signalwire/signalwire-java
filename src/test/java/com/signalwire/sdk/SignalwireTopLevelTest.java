...
/**
 * Tests for the top-level convenience entry points exposed on the {@link
 * com.signalwire.sdk.Signalwire} static utility class — {@code RestClient}, {@code registerSkill},
 * {@code addSkillDirectory}, {@code listSkillsWithParams}. These mirror Python's package-level
...
class SignalwireTopLevelTest {
  ...
  void restClientFromPositionalCredentials() {
    RestClient client =
        Signalwire.RestClient(
            Arrays.asList("proj", "tok", "pos.signalwire.com"), Collections.emptyMap());
    assertNotNull(client);
  }
  ...
  /**
   * Minimal SkillBase implementation for the registerSkill test. SkillBase is an interface; we
   * provide a stub that the singleton registry's {@code Supplier<SkillBase>} factory can
