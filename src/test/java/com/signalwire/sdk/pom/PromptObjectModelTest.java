...
/**
 * Java parity tests for {@link PromptObjectModel}, mirroring {@code
 * signalwire-python/tests/unit/pom/test_pom_object_model.py} and extending with byte-exact
 * rendered-shape assertions taken directly from Python's {@code render_markdown} / {@code
 * render_xml} / {@code to_yaml}.
 */
class PromptObjectModelTest {
  ...
  void onlyFirstSectionMayHaveNullTitle() {
    ...
    assertThrows(IllegalArgumentException.class, () -> pom.addSection(null, "second body"));
  }
  ...
  void addPomAsSubsectionUnknownTitleThrows() {
    ...
    assertThrows(IllegalArgumentException.class, () -> host.addPomAsSubsection("Missing", guest));
  }
