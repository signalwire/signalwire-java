...
class ConciergeTest {
  ...
  void testHasTools() {
    List<Map<String, Object>> amenities =
        List.of(ConciergeAgent.amenity("Pool", "Pool", "6-10", "1F", "Free"));
  ...
  void testAmenityHelper() {
    Map<String, Object> amenity =
        ConciergeAgent.amenity("Gym", "Fitness center", "24/7", "B1", "Free");
  ...
  void testCheckAvailabilityExecution() {
    ...
    ConciergeAgent prefab = new ConciergeAgent("test", "Hotel", amenities);
    var result =
        prefab.getAgent().onFunctionCall("check_availability", Map.of("amenity", "Pool"), Map.of());
    assertNotNull(result);
  }
