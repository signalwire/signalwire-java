...
class FaqBotTest {
  ...
  void testCreation() {
    List<Map<String, Object>> faqs =
        List.of(FAQBotAgent.faq("Hours?", "9-5 M-F", List.of("hours", "open")));
  ...
  void testHasLookupFaqTool() {
    List<Map<String, Object>> faqs =
        List.of(FAQBotAgent.faq("Hours?", "9-5 M-F", List.of("hours")));
  ...
  void testLookupFaqExecution() {
    ...
    FAQBotAgent prefab = new FAQBotAgent("test", faqs);
    var result = prefab.getAgent().onFunctionCall("lookup_faq", Map.of("query", "hours"), Map.of());
  ...
  void testLookupFaqNoMatch() {
    List<Map<String, Object>> faqs = List.of(FAQBotAgent.faq("Hours?", "9-5", List.of("hours")));
    FAQBotAgent prefab = new FAQBotAgent("test", faqs);
    var result =
        prefab.getAgent().onFunctionCall("lookup_faq", Map.of("query", "zzz_nothing"), Map.of());
    assertNotNull(result);
  ...
  void testSwmlRendering() {
    List<Map<String, Object>> faqs = List.of(FAQBotAgent.faq("Q?", "A.", List.of("k")));
    FAQBotAgent prefab = new FAQBotAgent("test", faqs);
