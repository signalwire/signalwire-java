 ...
 *
 * <ul>
 *   <li>Simulated env vars visible through the injected {@link EnvProvider}.
 *   <li>Real {@link System#getenv()} is untouched at every step.
 *   <li>{@code SWML_PROXY_URL_BASE} in the real env is masked by the simulated view so Lambda URL
 *       generation is actually exercised.
 *   <li>Restoration works on exception paths (try/finally).
 *   <li>Unimplemented platforms error out cleanly — no silent server fallback.
 *   <li>Non-root routes appear in dumped SWML webhook URLs (regression for the Lambda
...
class SwaigTestSimulateServerlessTest {
  ...
  /**
   * Drive {@link SwaigTest#run(String[])} with a simulator backed by the given "real env" view.
   * Uses reflection to inject the real-env override into a fresh {@code SwaigTest} instance, since
   * we can't actually mutate {@link System#getenv()}.
   */
  ...
  void rejectsUnimplementedPlatformAzure() throws Exception {
    int code =
        SwaigTest.run(
            new String[] {
              RootAgentFixture.class.getName(), "--simulate-serverless", "azure", "--dump-swml"
            });
    String streams = restoreStreams();
  ...
  void simulatedEnvIsVisibleToAgentViaInjectedProvider() throws Exception {
    int code =
        SwaigTest.run(
            new String[] {
              RootAgentFixture.class.getName(), "--simulate-serverless", "lambda", "--dump-swml"
            });
    String streams = restoreStreams();
    ...
    assertNotNull(
        seen.get("AWS_LAMBDA_FUNCTION_NAME"),
        "simulated env should expose AWS_LAMBDA_FUNCTION_NAME");
    assertNotNull(seen.get("AWS_REGION"), "simulated env should expose AWS_REGION");
  }
  ...
  void realSystemGetenvIsUntouchedDuringSimulation() throws Exception {
    ...
    assertEquals(pathBefore, System.getenv("PATH"), "PATH must not be mutated by the simulator");
    assertEquals(
        lambdaBefore,
  ...
  void dumpSwmlEmitsAiVerbWithLambdaStyleWebhookUrl() throws Exception {
    ...
    String streams = restoreStreams();
    assertEquals(0, code, streams);
    assertTrue(out.contains("\"sections\""), "dumped SWML should contain 'sections'. out=" + out);
    assertTrue(
        out.contains("web_hook_url"), "dumped SWML should contain a web_hook_url. out=" + out);
    assertTrue(
        out.contains(".lambda-url.") || out.contains("lambda-url."),
  ...
  void bareSimulationRendersSwmlAndExitsZero() throws Exception {
    int code =
        SwaigTest.run(
            new String[] {RootAgentFixture.class.getName(), "--simulate-serverless", "lambda"});
    String streams = restoreStreams();
    assertEquals(0, code, streams);
    assertTrue(
        stdout.toString(StandardCharsets.UTF_8).contains("sections"),
  ...
  void simulatedProxyUrlBaseIsMaskedFromRealEnv() throws Exception {
    ...
    assertFalse(
        out.contains("real-proxy.example.com"),
    ...
    assertTrue(
        err.contains("SWML_PROXY_URL_BASE") && err.contains("masking"),
  ...
  void routePreservedWhenSwmlProxyUrlBaseMasked() throws Exception {
    ...
    assertFalse(
        out.contains("real-proxy.example.com"), "real-env proxy base must not appear. out=" + out);
  }
  ...
  void realEnvSurvivesExceptionPath() throws Exception {
    ...
    assertEquals(before, after, "real env must survive the tool-exception path. " + streams);
  }
  ...
  void realEnvSurvivesSuccessPath() throws Exception {
    ...
    Map<String, String> after = snapshotRelevantEnv();
    assertEquals(before, after, "real env must survive the success path. " + streams);
  }

  /**
   * Snapshot of process env vars relevant to simulation. If simulation ever mutates {@link
   * System#getenv()} these values would diverge.
   */
  ...
  void simulateServerlessRequiresAgentClass() throws Exception {
    int code = SwaigTest.run(new String[] {"--simulate-serverless", "lambda", "--dump-swml"});
    String streams = restoreStreams();
    assertNotEquals(0, code);
    assertTrue(
        stderr.toString(StandardCharsets.UTF_8).contains("agent-class"),
  ...
  void simulateServerlessRejectsCombinationWithUrl() throws Exception {
    ...
    assertNotEquals(0, code);
    assertTrue(
        stderr.toString(StandardCharsets.UTF_8).contains("--url")
  ...
  void simulateServerlessRejectsListToolsForNow() throws Exception {
    int code =
        SwaigTest.run(
            new String[] {
              RootAgentFixture.class.getName(), "--simulate-serverless", "lambda", "--list-tools"
            });
    String streams = restoreStreams();
    assertNotEquals(0, code);
    assertTrue(
        stderr.toString(StandardCharsets.UTF_8).contains("--list-tools"),
  ...
  void missingAgentClassProducesClearError() throws Exception {
    int code =
        SwaigTest.run(
            new String[] {"com.does.not.Exist", "--simulate-serverless", "lambda", "--dump-swml"});
    String streams = restoreStreams();
    assertNotEquals(0, code);
    assertTrue(
        stderr.toString(StandardCharsets.UTF_8).contains("not found")
