 ...
 *
 * <ul>
 *   <li>Platform parsing — rejects unimplemented platforms with a clear error.
 *   <li>Layered {@link EnvProvider} — simulated keys win, real env is fallback, masked keys always
 *       return null.
 *   <li>{@code SWML_PROXY_URL_BASE} is masked even if the real env has it.
 *   <li>The layered env makes {@link ExecutionMode#detect(EnvProvider)} see LAMBDA.
 * </ul>
 */
class ServerlessSimulatorTest {
  ...
  void parsePlatformAcceptsLambda() {
    assertEquals(ServerlessSimulator.Platform.LAMBDA, ServerlessSimulator.parsePlatform("lambda"));
    assertEquals(ServerlessSimulator.Platform.LAMBDA, ServerlessSimulator.parsePlatform("LAMBDA"));
    assertEquals(
        ServerlessSimulator.Platform.LAMBDA, ServerlessSimulator.parsePlatform(" lambda "));
  }
  ...
  void parsePlatformRejectsUnimplementedPlatforms() {
    for (String unsupported :
      ...
      assertTrue(
          ex.getMessage().contains("Unsupported platform"),
  ...
  void parsePlatformRejectsNull() {
    assertThrows(IllegalArgumentException.class, () -> ServerlessSimulator.parsePlatform(null));
  }

  @Test
  void lambdaPresetHasExpectedKeys() {
    Map<String, String> preset = ServerlessSimulator.presetFor(ServerlessSimulator.Platform.LAMBDA);
  ...
  void layeredEnvProviderOverlaysSimulatedOnTopOfRealEnv() {
    ...
    ServerlessSimulator sim =
        new ServerlessSimulator(ServerlessSimulator.Platform.LAMBDA, real, Map.of());
  ...
  void layeredEnvMasksProxyUrlBaseEvenIfRealEnvHasIt() {
    EnvProvider real = envOf(Map.of("SWML_PROXY_URL_BASE", "https://real-proxy.example.com"));
    ServerlessSimulator sim =
        new ServerlessSimulator(ServerlessSimulator.Platform.LAMBDA, real, Map.of());

    assertTrue(
        sim.proxyUrlBaseMaskedFromRealEnv(),
        "simulator should report that it's masking the real SWML_PROXY_URL_BASE");

    EnvProvider layered = sim.buildEnvProvider();
    assertNull(
        layered.get("SWML_PROXY_URL_BASE"),
  ...
  void layeredEnvUserOverridesWinOverPreset() {
    ...
    ServerlessSimulator sim =
        new ServerlessSimulator(ServerlessSimulator.Platform.LAMBDA, EnvProvider.SYSTEM, overrides);
