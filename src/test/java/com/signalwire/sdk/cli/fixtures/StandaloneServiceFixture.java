...
/**
 * Test fixture for the {@code --class} file-loader mode: a non-AgentBase {@link Service} subclass
 * that registers a single SWAIG tool in its constructor. Exercises the same shape as {@code
 * examples/SwmlServiceSwaigStandalone.java} (the production example the file-loader path is meant
 * to support).
 *
 * <p>The CLI invokes the public no-arg constructor via reflection — no static initialiser, no
 * {@code main()}.
 */
