...
/**
 * Parity tests for SchemaUtils — mirrors Python's tests/unit/utils/ test_schema_utils.py and the TS
 * / Perl reference implementations.
 *
 * <p>Each public method is exercised; assertions check shape (not just non-nullness) so the
...
class SchemaUtilsParityTest {
  ...
  void testValidateVerb_UnknownVerb() {
    ...
    Map.Entry<Boolean, List<String>> res =
        su.validateVerb("not_a_real_verb", new LinkedHashMap<>());
    assertFalse(res.getKey());
    assertEquals(1, res.getValue().size());
    assertTrue(
        res.getValue().get(0).contains("Unknown verb"),
  ...
  void testGenerateMethodSignatureContainsKwargs() {
    ...
    assertTrue(
        sig.startsWith("def answer("),
        "expected signature to start with 'def answer(', got: " + sig);
    assertTrue(sig.contains("**kwargs"), "expected **kwargs in signature, got: " + sig);
  }
  ...
  void testGenerateMethodBody() {
    ...
    assertTrue(
        body.contains("self.add_verb('answer'"),
        "expected body to call self.add_verb('answer'), got: " + body);
    assertTrue(body.contains("config = {}"), "expected body to init config={}, got: " + body);
  }
  ...
  void testServiceSchemaUtilsAccessor() {
    ...
    assertNotNull(su);
    assertFalse(su.getAllVerbNames().isEmpty(), "expected verbs from Service-bound SchemaUtils");
  }

  @Test
  void testSchemaValidationError() {
    SchemaValidationError e =
        new SchemaValidationError("ai", List.of("missing prompt", "bad type"));
    assertEquals("ai", e.getVerbName());
    assertEquals(2, e.getErrors().size());
    assertTrue(
        e.getMessage().contains("ai"), "expected 'ai' in error message, got: " + e.getMessage());
    assertTrue(
        e.getMessage().contains("missing prompt"),
