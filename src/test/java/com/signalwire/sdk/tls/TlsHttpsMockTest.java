...
/**
 * TLS capability test, quadrant 2 of 3: prove a JDK {@link HttpClient} performs a REAL verified
 * {@code https://} GET against the shared mock_signalwire (whole app served over HTTPS in {@code
 * --tls} mode), and gets a genuine JSON response back.
 *
 * <p>CA trust: the client is built with {@code HttpClient.newBuilder().sslContext(...)} where the
 * {@link SSLContext} trusts ONLY {@code ca.crt}, assembled via an in-memory {@link
 * java.security.KeyStore} → {@code TrustManagerFactory} ({@link TlsSupport#trustingSslContext}).
 * Java does NOT read the OS trust store, so this explicit context is what makes the harness's
 * CA-signed leaf cert verify. NO trust-all TrustManager, NO transport mock — a {@code data} array
 * can only come back over a completed, CA-verified TLS session.
 *
 * <p>This drives the JDK HttpClient directly (the same transport the SDK's REST {@code HttpClient}
 * is built on) because the SDK's REST client builds its {@code java.net.http.HttpClient} internally
 * with no SSLContext hook; pointing a custom-CA context at the production REST client is not
 * possible today (see the report's SDK-gap finding). The GET targets the same spec-backed {@code
 * /api/relay/rest/addresses} collection the SDK's {@code RestClient.addresses().list()} hits.
 *
 * <p>Negative control: the same GET issued by a client built from an EMPTY trust store must fail
 * the handshake, proving the cert is actually verified.
 */
class TlsHttpsMockTest {
  ...
  void httpsGetReturnsRealJson() throws Exception {
    ...
    try (TlsSupport.MockHandle mock = TlsSupport.spawnTlsMockSignalwire(certs)) {
      ...
      String basic =
          Base64.getEncoder().encodeToString("test_proj:test_tok".getBytes(StandardCharsets.UTF_8));
      HttpRequest req =
      ...
      assertEquals(200, resp.statusCode(), "https GET status; body=" + resp.body());

      Map<String, Object> body =
          GSON.fromJson(resp.body(), new TypeToken<Map<String, Object>>() {}.getType());
      assertNotNull(body, "empty body over https");
      assertTrue(
          body.containsKey("data"), "https response missing 'data' key; got " + body.keySet());
  ...
  /**
   * A client built from an EMPTY trust store must fail the TLS handshake against the harness's
   * CA-signed leaf cert, proving real verification.
   */
