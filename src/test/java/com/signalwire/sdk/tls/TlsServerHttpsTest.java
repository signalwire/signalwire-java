...
/**
 * TLS capability test, quadrant 3 of 3 (the server side): prove the SDK's own
 * {@link AgentServer} serves a REAL verified HTTPS endpoint.
 *
 * <p>This exercises the port-specific server-TLS fix. The JDK's
 * {@code com.sun.net.httpserver.HttpServer} (what AgentServer used) has no TLS;
 * the SDK now binds {@code com.sun.net.httpserver.HttpsServer} +
 * {@code setHttpsConfigurator(new HttpsConfigurator(sslContext))} when TLS is
 * configured — via {@link AgentServer#enableTls(String, String)} (explicit
 * cert/key, the precedence option) or the {@code SWML_SSL_*} environment
 * variables, mirroring Python's {@code AgentServer.run()}.
 *
 * <p>The test starts the server with the harness's CA-signed leaf
 * ({@code server.crt}/{@code server.key}, SAN localhost/127.0.0.1) in a
 * background thread, then reaches its unauthenticated {@code /health} route
 * from an in-test JDK {@link HttpClient} that trusts the test CA
 * ({@link TlsSupport#trustingSslContext}). It asserts a 200, a non-null
 * {@link SSLSession} on the response (the request really went over TLS), and a
 * {@code status=healthy} body. CA trust is the in-memory KeyStore →
 * TrustManagerFactory path; NO trust-all TrustManager.
 *
 * <p>Negative control: a client built from an EMPTY trust store must be
 * rejected, proving the SDK server presents a cert that is actually verified.
 */
class TlsServerHttpsTest {
    ...
    void agentServerServesVerifiedHttps() throws Exception {
        ...
        assertTrue(session.isPresent(),
                "response has no SSLSession; the request did not go over TLS");
        ...
        assertEquals("healthy", body.get("status"),
                "https /health body status; got " + body);
    ...
    /**
     * A client from an EMPTY trust store must fail the handshake against the
     * SDK server's CA-signed leaf cert, proving the cert is actually verified.
     */
