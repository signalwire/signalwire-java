...
class WebSearchSkillTest {
    ...
    void testSuccessfulResponseWrappedWithPrefixAndPostfix() throws Exception {
        ...
        try (StubServer stub = StubServer.startWithScrapableItem(
                "Title One", "Snippet here", "page-one",
    ...
    void testSuccessfulResponseWithoutPrefixPostfixIsUnwrapped() throws Exception {
        try (StubServer stub = StubServer.startWithScrapableItem(
            ...
            assertTrue(response.startsWith("Quality web search results for"),
    ...
    void testSnippetsOnlySkipsPageFetch() throws Exception {
        ...
        try (StubServer stub = StubServer.startWithScrapableItem(
                "SnipTitle", "snippet body text", "should-not-be-fetched",
            ...
            assertEquals(0, stub.contentHits.get(),
                    "snippets_only must not fetch any page; hits=" + stub.contentHits.get());
            assertTrue(response.contains("Snippet-only results"),
                    "snippets_only must format snippet block: " + response);
            assertTrue(response.contains("snippet body text"),
                    "snippet text must be present: " + response);
            assertFalse(response.contains("SHOULD NOT BE SCRAPED"),
    ...
    void testDeadlineTruncatesToSnippetFallback() throws Exception {
        ...
        try (StubServer stub = StubServer.startWithSlowItem(
                "SlowTitle", "slow snippet body", "slow-page", sleepMs)) {
            WebSearchSkill skill = new WebSearchSkill();
            skill.setup(Map.of(
                    "api_key", "k",
                    "search_engine_id", "cx",
            ...
            assertTrue(elapsedMs < sleepMs - 2_000,
                    "must return before the slow page completes; elapsed=" + elapsedMs + "ms");
            assertTrue(elapsedMs >= (long) (deadlineSecs * 1000) - 200,
            ...
            assertTrue(response.contains("slow snippet body"),
                    "snippet text must be present in fallback: " + response);
            assertFalse(response.contains("EMPTY-NO-RESULTS-SHOULD-NOT-APPEAR"),
    ...
    void testSequentialModeBoundsLatencyAndFallsBack() throws Exception {
        ...
        try (StubServer stub = StubServer.startWithSlowItem(
                "SeqTitle", "seq snippet body", "seq-slow", sleepMs)) {
            WebSearchSkill skill = new WebSearchSkill();
            skill.setup(Map.of(
                    "api_key", "k",
                    "search_engine_id", "cx",
                    "overall_deadline", 20.0,
                    "per_page_timeout", perPageSecs,
                    "parallel_scrape", false
            ));
            long start = System.nanoTime();
            String response = invokeHandler(skill, "seq query");
            long elapsedMs = (System.nanoTime() - start) / 1_000_000L;
            assertTrue(elapsedMs < sleepMs - 1_000,
    ...
    void testPerPageTimeoutAbandonsSlowPage() throws Exception {
        ...
        try (StubServer stub = StubServer.startWithSlowItem(
                "PptTitle", "ppt snippet body", "ppt-slow", sleepMs)) {
            WebSearchSkill skill = new WebSearchSkill();
            skill.setup(Map.of(
                    "api_key", "k",
                    "search_engine_id", "cx",
    ...
    void testSnippetFallbackWhenScrapeYieldsEmpty() throws Exception {
        ...
        try (StubServer stub = StubServer.startWith404Content(
                "FbTitle", "fallback snippet text", "missing-page")) {
            WebSearchSkill skill = new WebSearchSkill();
            skill.setup(Map.of(
                    "api_key", "k",
                    "search_engine_id", "cx",
            ...
            assertFalse(response.contains("EMPTY-SHOULD-NOT-APPEAR"),
    ...
    void testParameterSchemaAdvertisesAllSixParams() {
        ...
        for (String key : List.of("response_prefix", "response_postfix",
                "per_page_timeout", "overall_deadline", "parallel_scrape", "snippets_only")) {
    ...
    void testEverySetupParamIsAdvertised() {
        ...
        WebSearchSkill skill = new WebSearchSkill();
        Map<String, Object> schema = skill.getParameterSchema();
        for (String key : List.of("tool_name", "num_results", "no_results_message",
                "response_prefix", "response_postfix", "per_page_timeout",
                "overall_deadline", "parallel_scrape", "snippets_only")) {
            assertTrue(schema.containsKey(key),
                    "setup() reads '" + key + "' but schema omits it");
        }
    }
    ...
    /**
     * Minimal in-process HTTP server that mimics Google Custom Search responses
     * AND serves the candidate content pages, so page scrapes are deterministic
     * and offline. The CSE item's {@code link} points back at this server's own
     * {@code /content/<id>} path. Sets WEB_SEARCH_BASE_URL to redirect the CSE
     * call here; restores the prior value on close. A {@link #contentHits}
     * counter records how many content fetches actually started, so a test can
    ...
    private static class StubServer implements AutoCloseable {
        ...
        static StubServer startWithScrapableItem(String title, String snippet,
                                                 String contentId, String contentBody) throws IOException {
            return build(title, snippet, contentId, contentBody, 0L, 200);
        }

        /** CSE returns one item whose content page sleeps {@code sleepMs}. */
        static StubServer startWithSlowItem(String title, String snippet,
                                            String contentId, long sleepMs) throws IOException {
            return build(title, snippet, contentId,
                    "<html><body>slow body, should be abandoned</body></html>", sleepMs, 200);
        }

        /** CSE returns one item whose content page 404s (empty scrape). */
        static StubServer startWith404Content(String title, String snippet,
        ...
        /**
         * Build a stub serving (1) the CSE endpoint with a single item whose
         * link points back at this server's {@code /content/<id>} path, and
         * (2) that content page (optionally sleeping {@code sleepMs} and/or
         * returning {@code contentStatus}).
         */
        private static StubServer build(String title, String snippet, String contentId,
                                        String contentBody, long sleepMs,
                                        int contentStatus) throws IOException {
            HttpServer s = HttpServer.create(new InetSocketAddress("127.0.0.1", 0), 0);
            ...
            String cseBody = String.format(
                    "{\"items\":[{\"title\":\"%s\",\"snippet\":\"%s\",\"link\":\"%s\"}]}",
                    title, snippet, link);
            registerCse(s, cseBody);

            s.createContext(contentPath, (HttpExchange ex) -> {
