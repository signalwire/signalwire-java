...
class VerbTest {
  ...
  @BeforeEach
  void setUp() {
    agent = AgentBase.builder().name("verb-test").authUser("u").authPassword("p").build();
  ...
  void testAnswerVerbHasMaxDuration() {
    Map<String, Object> swml = agent.renderSwml("http://localhost:3000");
    List<Map<String, Object>> main = getMain(swml);
    Map<String, Object> answerVerb =
        main.stream().filter(v -> v.containsKey("answer")).findFirst().orElseThrow();
