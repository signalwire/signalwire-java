...
/**
 * Proves the three Tier-1 SWAIG {@link FunctionResult} closed-set enums —
 * {@link RecordDirection} ({@code record_call} direction), {@link TapDirection}
 * ({@code tap} direction) and {@link Codec} ({@code tap} codec) — each behave
 * identically to the equivalent bare string they keep parity with.
 *
 * <p>For every enum: (a) each constant's wire value; (b) the enum overload and
 * the equivalent String produce the BYTE-IDENTICAL {@code record_call}/{@code tap}
 * action (driving the real method → the built Map, asserting the serialized SWML
 * params); (c) every value round-trips through {@code fromWire}; (d) an
 * out-of-set String is still rejected by the method's existing validation.
 *
 * <p>No mocks: real {@link FunctionResult} rendering the real SWML document.
 *
 * <p>The three sets are deliberately DISTINCT vocabularies and are never shared
 * (the 3-vocabulary trap): {@code record_call} uses {@code listen} where
 * {@code tap} uses {@code hear}; the SWAIG tap {@code Codec} is a 2-value set,
 * not the larger RELAY connect/stream codec superset. The cross-rejection tests
 * below pin that the {@code fromWire} parsers refuse the other set's member.
 */
class MediaEnumTest {
    ...
    void recordDirectionEnumEqualsStringByteIdentical() {
        ...
        FunctionResult viaEnum = new FunctionResult("Recording")
                .recordCall("ctrl-1", true, "mp3", RecordDirection.LISTEN);
        FunctionResult viaString = new FunctionResult("Recording")
                .recordCall("ctrl-1", true, "mp3", "listen");
    ...
    void recordDirectionEnumEqualsStringForEveryValue() {
        for (RecordDirection d : RecordDirection.values()) {
            FunctionResult viaEnum = new FunctionResult("Recording")
                    .recordCall("c", false, "wav", d);
            FunctionResult viaString = new FunctionResult("Recording")
                    .recordCall("c", false, "wav", d.getValue());
            assertEquals(viaString.toMap(), viaEnum.toMap(),
    ...
    void recordDirectionFromWireRoundTripsEveryValue() {
        for (RecordDirection d : RecordDirection.values()) {
            assertSame(d, RecordDirection.fromWire(d.getValue()),
                    "round-trip failed for " + d.name());
        }
    }
    ...
    void recordCallRejectsOutOfSetDirectionString() {
        ...
        assertTrue(ex.getMessage().contains("speak")
                        && ex.getMessage().contains("listen") && ex.getMessage().contains("both"),
    ...
    void tapDirectionEnumEqualsStringByteIdentical() {
        ...
        FunctionResult viaEnum = new FunctionResult("Tapping")
                .tap("wss://x.example", "t1", TapDirection.HEAR, "PCMU");
        FunctionResult viaString = new FunctionResult("Tapping")
                .tap("wss://x.example", "t1", "hear", "PCMU");
    ...
    void tapDirectionEnumEqualsStringForEveryValue() {
        for (TapDirection d : TapDirection.values()) {
            FunctionResult viaEnum = new FunctionResult("Tapping")
                    .tap("wss://x.example", "t1", d, "PCMU");
            FunctionResult viaString = new FunctionResult("Tapping")
                    .tap("wss://x.example", "t1", d.getValue(), "PCMU");
            assertEquals(viaString.toMap(), viaEnum.toMap(),
    ...
    void tapDirectionFromWireRoundTripsEveryValue() {
        for (TapDirection d : TapDirection.values()) {
            assertSame(d, TapDirection.fromWire(d.getValue()),
                    "round-trip failed for " + d.name());
        }
    }
    ...
    void tapRejectsOutOfSetDirectionString() {
        ...
        assertTrue(ex.getMessage().contains("speak") && ex.getMessage().contains("hear")
    ...
    void codecEnumEqualsStringByteIdentical() {
        ...
        FunctionResult viaEnum = new FunctionResult("Tapping")
                .tap("wss://x.example", "t1", "both", Codec.PCMA);
        FunctionResult viaString = new FunctionResult("Tapping")
                .tap("wss://x.example", "t1", "both", "PCMA");
    ...
    void codecEnumEqualsStringForEveryValue() {
        for (Codec c : Codec.values()) {
            FunctionResult viaEnum = new FunctionResult("Tapping")
                    .tap("wss://x.example", "t1", "speak", c);
            FunctionResult viaString = new FunctionResult("Tapping")
                    .tap("wss://x.example", "t1", "speak", c.getValue());
            assertEquals(viaString.toMap(), viaEnum.toMap(),
                    "enum vs string diverged for Codec." + c.name());
        }
    }
    ...
    void tapFullyTypedOverloadMatchesStrings() {
        ...
        FunctionResult viaEnum = new FunctionResult("Tapping")
                .tap("wss://x.example", "t1", TapDirection.SPEAK, Codec.PCMA);
        FunctionResult viaString = new FunctionResult("Tapping")
                .tap("wss://x.example", "t1", "speak", "PCMA");
        assertEquals(viaString.toMap(), viaEnum.toMap());
    }
    ...
    void codecFromWireRoundTripsEveryValue() {
        for (Codec c : Codec.values()) {
            assertSame(c, Codec.fromWire(c.getValue()),
    ...
    void tapRejectsOutOfSetCodecString() {
        ...
        assertTrue(ex.getMessage().contains("PCMU") && ex.getMessage().contains("PCMA"),
                "unexpected message: " + ex.getMessage());
    }
}
