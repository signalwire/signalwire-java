...
/**
 * Tests for web/HTTP configuration: webhook URLs, proxy, query params, dynamic config callback,
 * post-prompt URL, debug routes.
 */
class WebTest {
  ...
  @BeforeEach
  void setUp() {
    agent = AgentBase.builder().name("web-test").authUser("u").authPassword("p").build();
  ...
  void testSetDynamicConfigCallback() {
    agent.setDynamicConfigCallback(
        (queryParams, bodyParams, headers, agentRef) -> {
          // No-op callback
        });
    assertNotNull(agent);
  }
  ...
  /**
   * Regression test mirroring the bug fixed in Python and TypeScript SDKs: when a non-root route is
   * combined with a proxy base URL (such as a Lambda Function URL), the rendered webhook URL must
   * include the agent's route before {@code /swaig}. The proxy base must NOT be used bare.
   */
  @Test
  @SuppressWarnings("unchecked")
  void testRoutePreservedWithProxyBase() {
    ...
    Map<String, Object> swml = routeAgent.renderSwml("https://xyz.lambda-url.us-east-1.on.aws");
    Map<String, Object> ai = extractAi(swml);
    Map<String, Object> swaig = (Map<String, Object>) ai.get("SWAIG");
    List<Map<String, Object>> fns = (List<Map<String, Object>>) swaig.get("functions");
    String url = (String) fns.get(0).get("web_hook_url");
  ...
  void testRoutePreservedInPostPromptUrl() {
    ...
    Map<String, Object> ai = extractAi(swml);
    assertEquals(
        "https://xyz.lambda-url.us-east-1.on.aws/my-agent/post_prompt", ai.get("post_prompt_url"));
  }
