...
/**
 * Tests proving SWML Service can host SWAIG functions and serve a non-agent SWML doc (e.g.
 * ai_sidecar) without subclassing AgentBase. This is the contract that lets sidecar / non-agent
 * verbs reuse the SWAIG dispatch surface that previously lived only on AgentBase.
 */
class SwmlServiceSwaigTest {
  ...
  void testServiceHasSwaigMethods() throws Exception {
    ...
    assertNotNull(
        svc.getClass()
            .getMethod("onFunctionCall", String.class, java.util.Map.class, java.util.Map.class));
  }
  ...
  void testSidecarPatternEmitsVerbAndRegistersTool() {
    ...
    var result = svc.onFunctionCall("lookup_competitor", Map.of("competitor", "ACME"), Map.of());
    assertNotNull(result);
