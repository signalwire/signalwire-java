 ...
 *
 * <p>DataMap tools execute on SignalWire servers without requiring webhook endpoints. Supports
 * variable expansion: {@code ${args.param}}, {@code ${response.field}}, {@code ${global_data.key}},
 * {@code ${foreach.item}}.
 *
...
public class DataMap {
   ...
   *
   * <p>The description string is rendered into the OpenAI tool schema {@code description} field on
   * every LLM turn. The model reads it to decide WHEN to call this tool. A vague {@code purpose()}
   * is the #1 cause of "the model has the right tool but doesn't call it" failures with data-map
   * tools.
   *
   * <h3>Bad vs good</h3>
   *
  ...
  /**
   * Alias for {@link #purpose(String)}; sets the LLM-facing tool description. This string is read
   * by the model to decide WHEN to call this tool. See {@link #purpose(String)} for bad-vs-good
   * examples.
   */
  ...
  /**
   * Add a parameter to this data-map tool — the {@code description} is LLM-FACING.
   *
   * <p>Each parameter description is rendered into the OpenAI tool schema under {@code
   * parameters.properties.<name>.description} and sent to the model. The model uses it to decide
   * HOW to fill in the argument from user speech. It is prompt engineering, not developer FYI.
   *
   * <h3>Bad vs good</h3>
   *
   * <pre>{@code
  ...
  public DataMap expression(
      String testValue, String pattern, FunctionResult output, FunctionResult nomatchOutput) {
  ...
  public Map<String, Object> toSwaigFunction() {
    ...
    paramSchema.put(
        "properties", paramProperties.isEmpty() ? Map.of() : new LinkedHashMap<>(paramProperties));
