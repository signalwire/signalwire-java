/*
 * Copyright (c) 2026 SignalWire
 *
 * Licensed under the MIT License.
 * See LICENSE file in the project root for full license information.
 */
package com.signalwire.sdk.aichat;

import java.util.Map;

/**
 * Options for {@link AIChatClient#createConversation}. {@code configUrl} is required; other fields
 * are optional. Build with {@link #builder()}.
 */
public final class CreateConversationOptions {

  private final String configUrl;
  private final String userMessage;
  private final Integer timeout;
  private final Boolean reinit;
  private final Map<String, Object> userMetadata;

  private CreateConversationOptions(Builder b) {
    this.configUrl = b.configUrl;
    this.userMessage = b.userMessage;
    this.timeout = b.timeout;
    this.reinit = b.reinit;
    this.userMetadata = b.userMetadata;
  }

  /**
   * A new builder.
   *
   * @return a builder.
   */
  public static Builder builder() {
    return new Builder();
  }

  /**
   * Config URL locating the agent config (required, wire {@code config_url}).
   *
   * @return the config URL.
   */
  public String getConfigUrl() {
    return configUrl;
  }

  /**
   * The opening user message to send with the create (wire {@code user_message}).
   *
   * @return the opening message, or {@code null}.
   */
  public String getUserMessage() {
    return userMessage;
  }

  /**
   * Conversation inactivity timeout in seconds (wire {@code conversation_timeout}).
   *
   * @return the timeout, or {@code null}.
   */
  public Integer getTimeout() {
    return timeout;
  }

  /**
   * Whether to reinitialize an existing conversation (wire {@code reinit}).
   *
   * @return {@code true} to reinit.
   */
  public boolean isReinit() {
    return reinit != null && reinit;
  }

  /**
   * Arbitrary caller metadata (wire {@code user_meta_data}).
   *
   * @return the metadata map, or {@code null}.
   */
  public Map<String, Object> getUserMetadata() {
    return userMetadata;
  }

  /** Builder for {@link CreateConversationOptions}. */
  public static final class Builder {
    private String configUrl;
    private String userMessage;
    private Integer timeout;
    private Boolean reinit;
    private Map<String, Object> userMetadata;

    private Builder() {}

    /**
     * @param configUrl the config URL locating the agent config (required).
     * @return this builder.
     */
    public Builder configUrl(String configUrl) {
      this.configUrl = configUrl;
      return this;
    }

    /**
     * @param userMessage the opening user message.
     * @return this builder.
     */
    public Builder userMessage(String userMessage) {
      this.userMessage = userMessage;
      return this;
    }

    /**
     * @param timeout the conversation inactivity timeout in seconds.
     * @return this builder.
     */
    public Builder timeout(Integer timeout) {
      this.timeout = timeout;
      return this;
    }

    /**
     * @param reinit whether to reinitialize an existing conversation.
     * @return this builder.
     */
    public Builder reinit(boolean reinit) {
      this.reinit = reinit;
      return this;
    }

    /**
     * @param userMetadata arbitrary caller metadata.
     * @return this builder.
     */
    public Builder userMetadata(Map<String, Object> userMetadata) {
      this.userMetadata = userMetadata;
      return this;
    }

    /**
     * Build the options.
     *
     * @return the built options.
     */
    public CreateConversationOptions build() {
      return new CreateConversationOptions(this);
    }
  }
}
