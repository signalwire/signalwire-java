...
/**
 * REST namespace for the Video API: rooms, room sessions/recordings, conferences, conference
 * tokens, and individual streams.
 *
 * <p>Mirrors {@code signalwire.rest.namespaces.video.VideoNamespace}. The stream sub-resources hang
 * off rooms and conferences; the top-level {@code streams} resource exists for CRUD on individual
...
public class VideoNamespace {
  ...
  /**
   * Legacy alias for {@link #roomRecordings()}; previous releases of the Java port exposed
   * video.recordings(). Kept for backwards compatibility.
   */
  ...
  /**
   * Top-level video room recordings collection: list, get, delete, plus a per-recording events
   * sub-collection.
   */
  ...
  public static class VideoConferences extends CrudResource {
    ...
    public Map<String, Object> listConferenceTokens(String conferenceId) {
      return getHttpClient().get(getBasePath() + "/" + conferenceId + "/conference_tokens");
    }

    public Map<String, Object> listConferenceTokens(
        String conferenceId, Map<String, String> queryParams) {
      return getHttpClient()
          .get(getBasePath() + "/" + conferenceId + "/conference_tokens", queryParams);
    }
    ...
    public Map<String, Object> listStreams(String conferenceId, Map<String, String> queryParams) {
      return getHttpClient().get(getBasePath() + "/" + conferenceId + "/streams", queryParams);
    }
