...
/**
 * REST-based call control. All commands are dispatched as {@code POST /api/calling/calls} with a
 * {@code "command"} field that names the operation, optionally an {@code "id"} field that names the
 * target call, and a {@code "params"} object containing keyword arguments. The mock server (and the
 * real Calling API) matches commands against the registered routes via the {@code command} field.
 */
public class CallingNamespace {
  ...
  /**
   * Dispatches a calling command. {@code callId} may be null when the command targets the resource
   * collection (e.g. {@code dial} / {@code update}); {@code params} is a kwargs-shaped map.
   */
