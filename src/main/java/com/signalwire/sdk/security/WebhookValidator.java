 ...
 *       with optional {@code bodySHA256} query-param fallback for JSON-on-compat-surface.
 * </ul>
 *
 * <p>All signature comparisons use {@link MessageDigest#isEqual(byte[], byte[])} (constant-time) so
 * the secret cannot be leaked through timing differences.
 *
 * <p>Public API:
 *
 * <ul>
...
public final class WebhookValidator {
   ...
   *
   * <p>Tries Scheme A (hex HMAC-SHA1 over {@code url + rawBody}) first; on miss, tries Scheme B
   * (base64 HMAC-SHA1 over {@code url + sortedFormParams}) with both port-normalization variants of
   * the URL and an optional {@code bodySHA256} fallback for JSON-on-compat-surface.
   *
   * @param signingKey customer's Signing Key from the Dashboard. Must be non-null and non-empty;
   *     otherwise an {@link IllegalArgumentException} is thrown — that's a programming error, not a
   *     validation failure.
   ...
   *     empty.
   * @return {@code true} when the signature matches either scheme, otherwise {@code false}.
   * @throws IllegalArgumentException when {@code signingKey} is {@code null} or empty, or when
   *     {@code rawBody} is {@code null}.
   */
  public static boolean validateWebhookSignature(
    ...
    List<List<Map.Entry<String, String>>> paramShapes =
        List.of(parsedParams, List.of()); // empty params for JSON-on-compat
   ...
   *
   * <ul>
   *   <li>{@link String} → delegates to {@link #validateWebhookSignature(String, String, String,
   *       String)}.
   *   <li>{@link Map} or any {@link Iterable} of {@link Map.Entry} (or 2-element arrays / lists) →
   *       treats the value as pre-parsed form params and runs Scheme B directly with URL port
   *       normalization.
   *   <li>{@code null} → treated as an empty params map (Scheme B).
   ...
   * @throws IllegalArgumentException when {@code signingKey} is empty or {@code paramsOrRawBody} is
   *     of an unsupported type.
   */
  public static boolean validateRequest(
      String signingKey, String signature, String url, Object paramsOrRawBody) {
    if (signingKey == null || signingKey.isEmpty()) {
      throw new IllegalArgumentException("signingKey is required");
    }
    if (signature == null || signature.isEmpty()) {
  ...
  /**
   * Constant-time string compare via {@link MessageDigest#isEqual} on UTF-8 bytes. Returns {@code
   * false} on any unexpected condition rather than throwing — malformed/garbage signature inputs
   * must never crash the validator.
   */
  ...
  /**
   * Best-effort parse of an x-www-form-urlencoded body. Returns an empty list when the body doesn't
   * look like form data — the caller will then sign against {@code url + ""}.
   */
   ...
   *
   * <ul>
   *   <li>https + no port → input URL AND url with {@code :443}.
   *   <li>http + no port → input URL AND url with {@code :80}.
   *   <li>https + {@code :443} or http + {@code :80} → input URL AND url with port stripped.
   *   <li>Otherwise (any explicit non-standard port, or an unparseable URL) → just the input URL.
   * </ul>
   */
  private static List<String> candidateUrls(String url) {
    ...
    String standard =
        "https".equalsIgnoreCase(scheme) ? "443" : "http".equalsIgnoreCase(scheme) ? "80" : null;
  ...
  /**
   * If the URL carries {@code ?bodySHA256=<hex>}, verify {@code sha256_hex(rawBody)} matches.
   * Returns {@code true} when the param is absent (no constraint) or present and matches.
   */
  ...
  /**
   * Coerce a pre-parsed-params object into a list of (key, value) entries. Supports {@link Map}, an
   * {@link Iterable} of {@link Map.Entry}, or an {@link Iterable} of 2-element arrays / lists.
