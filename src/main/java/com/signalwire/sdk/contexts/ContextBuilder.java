 ...
 *
 * <p>A ContextBuilder owns one or more {@link Context}s; each context owns an ordered list of
 * {@link Step}s. Only one context and one step is active at a time. Per chat turn, the runtime
 * injects the current step's instructions as a system message, then asks the LLM for a response.
 *
 * <h2>Native tools auto-injected by the runtime</h2>
 *
 * <p>When a step (or its enclosing context) declares valid_steps or valid_contexts, the runtime
 * auto-injects two native tools so the model can navigate the flow:
 *
 * <ul>
 *   <li>{@code next_step(step: enum)} — present when valid_steps is set
 *   <li>{@code change_context(context: enum)} — present when valid_contexts is set
 * </ul>
 *
 * <p>A third native tool — {@code gather_submit} — is injected during gather_info questioning.
 * These three names are <b>reserved</b>: {@link #validate()} rejects any agent that defines a SWAIG
 * tool with one of these names. See {@link #RESERVED_NATIVE_TOOL_NAMES}.
 *
 * <h2>Function whitelisting (Step.setFunctions)</h2>
 *
 * <p>Each step may declare a functions whitelist. The whitelist is applied in-memory at the start
 * of each LLM turn. CRITICALLY: if a step does NOT declare a functions field, it INHERITS the
 * previous step's active set. See {@link Step#setFunctions(Object)} for details and examples.
 */
public class ContextBuilder {
  ...
  /**
   * Reserved tool names auto-injected by the runtime when contexts/steps are in use. User-defined
   * SWAIG tools must not collide with these names.
   *
   * <ul>
   *   <li>{@code next_step} / {@code change_context} are injected when valid_steps or
   *       valid_contexts is set so the model can navigate the flow.
   *   <li>{@code gather_submit} is injected while a step's gather_info is collecting answers.
   * </ul>
   */
  public static final Set<String> RESERVED_NATIVE_TOOL_NAMES =
      Collections.unmodifiableSet(
          new LinkedHashSet<>(Arrays.asList("next_step", "change_context", "gather_submit")));

  private final Map<String, Context> contexts;
  private final List<String> contextOrder;

  /**
   * Optional supplier returning the names of registered SWAIG tools, used by {@link #validate()} to
   * detect collisions with reserved native names. Wired by {@code AgentBase.defineContexts()}.
   */
  ...
  /**
   * Attach a supplier that returns registered SWAIG tool names so {@link #validate()} can check for
   * collisions with {@link #RESERVED_NATIVE_TOOL_NAMES}. Called internally by {@code
  ...
  /**
   * Remove all contexts, returning the builder to its initial state. Use this in a dynamic config
   * callback when you need to rebuild contexts from scratch for a specific request.
   *
   * @return this builder for chaining.
   */
    ...
    for (var entry : contexts.entrySet()) {
      ...
      if (context.getSteps().isEmpty()) {
        throw new IllegalStateException(
            "Context '" + contextName + "' must have at least one step");
      }
    }
            ...
            if (!seenKeys.add(q.getKey())) {
              throw new IllegalStateException(
                  "Step '"
                      ...
                      + "' in context '"
                      + contextName
                      + "' has duplicate gather_info question key '"
                      + q.getKey()
                      + "'");
            }
          }
            ...
            if ("next_step".equals(action)) {
              ...
              throw new IllegalStateException(
                  "Step '"
                      ...
                      + "' in context '"
                      + contextName
                      + "' has gather_info completion_action='"
                      + action
                      + "' but '"
                      + action
