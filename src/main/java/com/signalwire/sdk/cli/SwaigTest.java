 ...
 *
 * <p>Uses only JDK classes (java.net.http.HttpClient) -- no external dependencies.
 *
 * <p>Two modes:
 *
 * <ul>
 *   <li><b>URL mode</b> — hit a running agent HTTP server.
 *   <li><b>Simulation mode</b> — {@code --simulate-serverless &lt;platform&gt;} loads the agent
 *       class directly and routes invocation through the matching serverless adapter (e.g. {@link
 *       LambdaAgentHandler}) instead of the HTTP server. Uses an injected {@link EnvProvider} to
 *       mask the real process env with simulated values, since Java cannot mutate {@link
 *       System#getenv()}.
 * </ul>
 *
 * <p>Usage:
...
public class SwaigTest {
  ...
  /**
   * Run the CLI and return the exit code. Extracted so tests can drive {@code main} without calling
  ...
  /**
   * Inject an alternative "real env" for the simulator's fallback layer. Primarily for tests that
   * need to pretend the process env has {@code SWML_PROXY_URL_BASE} set without actually mutating
   * it. Null means "use {@link EnvProvider#SYSTEM}".
   *
  ...
  private void parseArgs(String[] args) {
    ...
    if (simulating && fileLoading) {
      throw new IllegalArgumentException("--class cannot be combined with --simulate-serverless");
    }
  ...
  /**
   * Sentinel signalling {@code --help} was requested. Distinct from {@link
  ...
  /**
   * Parse URL, extracting embedded auth credentials if present. Supports:
  ...
  /**
   * Load the class named by {@code --class} via reflection, instantiate it through its declared
   * no-arg constructor, verify it is a {@link com.signalwire.sdk.swml.Service}, and print every
   * registered SWAIG tool's name + description + parameters.
   *
   * <p>This is the introspection path for Service subclasses that don't emit an {@code <ai>} verb —
   * sidecar / standalone SWAIG hosts whose tools are invisible to {@code --list-tools --url}
   * because there's no AI verb to walk for {@code "function":"name"} markers.
   *
   * <p>No HTTP server is started; no socket is bound. The user is responsible for putting the
   * compiled class on the JVM classpath (typically via the {@code SWAIG_TEST_CLASSPATH} env var
   * honoured by {@code bin/swaig-test}).
   */
  private void runFileLoaderListTools() throws Exception {
    ...
    if (!(instance instanceof com.signalwire.sdk.swml.Service service)) {
      throw new IllegalArgumentException(
          fileLoaderClassName
              + " must extend com.signalwire.sdk.swml.Service "
              + "(got "
  ...
  /**
   * Drive the agent through the serverless adapter with a simulated env. Steps:
   *
   * <ol>
   *   <li>Parse + validate the platform (reject unimplemented ones).
  ...
  private void runSimulation() throws Exception {
    ServerlessSimulator.Platform platform = ServerlessSimulator.parsePlatform(simulatePlatformRaw);
    ...
    if (verbose) {
      ...
      System.err.println("[verbose] Simulated env keys: " + simulator.getSimulatedEnv().keySet());
  ...
  private void runLambdaSimulation(AgentBase agent, EnvProvider simEnv) throws Exception {
    ...
    Map<String, Object> event = buildApiGatewayV2Event("GET", path, basicAuthHeader(agent), null);
  ...
  private void emitResponse(LambdaResponse response) {
    if (response.getStatusCode() >= 400) {
      System.err.println("[warning] Lambda adapter returned HTTP " + response.getStatusCode());
    }
  ...
  private static Map<String, String> basicAuthHeader(AgentBase agent) {
    ...
    String creds =
        agent.getAuthUser()
            + ":"
            + (agent.getAuthPassword() != null ? agent.getAuthPassword() : "");
  ...
  private static Map<String, Object> buildApiGatewayV2Event(
  ...
  private static String buildSwaigRequestJson(String tool, Map<String, String> params) {
    ...
    if (!params.isEmpty()) {
      ...
      for (Map.Entry<String, String> e : params.entrySet()) {
        ...
        json.append("\"")
            .append(escapeJsonStatic(e.getKey()))
            .append("\":\"")
            .append(escapeJsonStatic(e.getValue()))
            .append("\"");
        first = false;
      }
      json.append("}]}");
  ...
  /**
   * Load the agent by reflection. The CLI expects the supplied class to expose a {@code public
   * static AgentBase} factory method whose name is one of (in order of preference): {@code
   * createAgent}, {@code buildAgent}, {@code newAgent}, {@code getAgent}. If the factory method
   * accepts an {@link EnvProvider} parameter, the simulated env is passed through directly.
   * Otherwise the returned agent is rebuilt with the simulated env applied to its {@link
   * AgentBase.Builder}.
   *
   * <p>Users whose agents live entirely inside a {@code main()} method should extract the build
   * logic into a public static factory — that's the documented pattern for making an agent testable
   * through this CLI (and is what {@code examples/LambdaAgent.java} already does with its {@code
   * buildAgent()} method).
   */
  private AgentBase loadAgent(String className, EnvProvider simEnv) throws Exception {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Agent class not found on classpath: "
              + className
    ...
    for (String name : List.of("createAgent", "buildAgent", "newAgent", "getAgent")) {
      try {
        ...
        if (result instanceof AgentBase agent) {
          if (verbose) {
            System.err.println(
                "[verbose] Loaded agent via " + className + "." + name + "() (no EnvProvider).");
          }
          return agent;
        }
        throw new IllegalStateException(className + "." + name + "() must return AgentBase");
      } catch (NoSuchMethodException ignored) {
        // Try next.
      }
    }
    ...
    throw new IllegalArgumentException(
        "Could not find a factory method on "
            + className
            + ". "
  ...
  private void doListTools() throws Exception {
    if (verbose)
      System.err.println("[verbose] GET " + baseUrl + " (fetching SWML to extract tools)");
  ...
  /**
   * Extract tool names from SWML JSON response. Looks for "function":"name" patterns within the
   * functions array.
   */
