 ...
 *
 * <p>Translates API Gateway (REST v1 or HTTP v2) / Lambda Function URL events into the same
 * dispatch logic the in-process HTTP server uses, without depending on the {@code
 * aws-lambda-java-events} typed models at runtime (the handler accepts plain {@code Map<String,
 * Object>} so the SDK stays dependency-light).
 *
 ...
 * }</pre>
 *
 * <p>The adapter dispatches based on the request path and the agent's configured route:
 *
 * <ul>
 *   <li>{@code GET /<route>} → returns rendered SWML
 *   <li>{@code POST /<route>/swaig} → executes the named SWAIG tool
 *   <li>{@code POST /<route>/post_prompt} → invokes the summary callback
 *   <li>{@code POST /<route>/mcp} → JSON-RPC 2.0 (if MCP enabled)
 *   <li>{@code GET /health}, {@code /ready} → health probes
 * </ul>
 *
 * <p>All webhook URLs generated inside the SWML document are built from the Lambda's Function URL
 * (via {@link LambdaUrlResolver}) or the {@code SWML_PROXY_URL_BASE} override, with the agent's
 * route always layered on top. This is the same invariant the HTTP server path enforces.
 */
public final class LambdaAgentHandler {
   ...
   *
   * @param event the raw Lambda event (API Gateway v1, v2, or Function URL payload). Null events
   *     are treated as a root GET.
   * @return response envelope ready to convert to {@code APIGatewayProxyResponseEvent} or
   *     equivalent.
   */
  public LambdaResponse handle(Map<String, Object> event) {
    try {
      ...
      return LambdaResponse.json(500, gson.toJson(Map.of("error", "Internal server error")));
    }
  }
  ...
  private LambdaResponse dispatch(Map<String, Object> event) {
    ...
    switch (rel) {
      ...
      default:
        return LambdaResponse.json(404, gson.toJson(Map.of("error", "Not found: " + rel)));
    }
  }
  ...
  private LambdaResponse handleSwml(Map<String, Object> event, String method) {
    ...
    if (cb != null) {
      ...
      Map<String, Object> body =
          "POST".equals(method) ? parseJsonBody(event) : new LinkedHashMap<>();
  ...
  private LambdaResponse unauthorised() {
    ...
    return new LambdaResponse(401, h, gson.toJson(Map.of("error", "Unauthorized")), false);
  }
   ...
   *
   * <p>{@code agentPath} is {@code ""} for root or e.g. {@code "/my-agent"}. Returns the remainder
   * (e.g. {@code "/swaig"}) or {@code null} if the path doesn't start with the agent's route.
   */
  ...
  private static Map<String, Object> parseJsonBody(Map<String, Object> event) {
    ...
    if (body instanceof String s) {
      ...
      if (Boolean.TRUE.equals(isB64)) {
        try {
          text = new String(Base64.getDecoder().decode(s), StandardCharsets.UTF_8);
