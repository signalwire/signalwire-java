...
/**
 * Resolve the base URL of a SignalWire agent running as an AWS Lambda function, from standard
 * Lambda environment variables.
 *
 * <p>Precedence — highest first:
 *
 * <ol>
 *   <li>{@code AWS_LAMBDA_FUNCTION_URL} — the Function URL explicitly assigned to this Lambda (the
 *       only variant that is guaranteed correct; synthetic fallbacks below assume the Function URL
 *       service was used and the function name matches the public subdomain, which is typically
 *       true but not guaranteed).
 *   <li>Synthesised {@code https://{AWS_LAMBDA_FUNCTION_NAME}.lambda-url.{AWS_REGION}.on.aws} —
 *       built when the Function URL env var is not present but the standard Lambda identity
 *       variables are.
 * </ol>
 *
 * <p>This class returns a bare origin (scheme + host). It intentionally does NOT append any route —
...
public final class LambdaUrlResolver {
  ...
  /**
   * Create a resolver backed by the given {@link EnvProvider}. Used in tests to avoid real env-var
   * mutation.
   *
   ...
   *
   * <p>Returns {@code null} if none of the expected env vars are set (which typically means we're
   * not actually running on Lambda). The caller should then fall back to its normal base-URL
   * detection.
   *
   * @return bare origin (e.g. {@code https://xyz.lambda-url.us-east-1.on.aws}), or {@code null}.
   */
