...
lines from several ports' run-ci (java lost all three §2.2 wirings); nothing caught
it because they aren't gates.

`porting-sdk/scripts/check_wired_modes.py --port java --repo .` greps run-ci.sh for
each required pattern below and FAILS LOUD on any missing one — so a future merge
that drops a wired mode reds the WIRED-MODES gate instead of silently shipping a
vacuous gate.
...
- `MOCK_RELAY_STRICT=1` — RELAY strict mode: the RELAY mock 400s any inbound frame that violates the wire spec, so a wrong frame fails LOUD (EXAMPLES-RUN / SNIPPET-RUN nightly gates run under it) instead of being silently journaled.
...
- `SW_BAN_BARE_IGNORE_KEYS` — bare-key ban armed (2.12): DOC-AUDIT rejects any bare, unqualified DOC_AUDIT_IGNORE key (a key that suppresses its token everywhere instead of a specific Class#member), so an over-broad ignore reds the gate.
