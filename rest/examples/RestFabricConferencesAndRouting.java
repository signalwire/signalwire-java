 ...
 * client.fabric().callFlows()}, {@code .swmlScripts()}, {@code .aiAgents()}, {@code
 * .subscribers().sipEndpoints()}, ...). Live video conference sessions are listed via {@code
...
public class RestFabricConferencesAndRouting {
  ...
  public static void main(String[] args) {
    ...
    try {
      var conference =
          client
              .fabric()
              .conferenceRooms()
              .create(Map.of("name", "team-standup", "max_members", 25, "record_on_start", true));
    ...
    try {
      ...
      // ResourceListResponse is a pure-data DTO with no toString(); concatenating it printed the
      // identity hash ("ResourceListResponse@4488aabb") instead of any result. The resources live
      // on `data`.
    ...
    System.out.println("\nCreating call flow...");
    try {
      var callFlow = client.fabric().callFlows().create(Map.of("title", "main-ivr"));
