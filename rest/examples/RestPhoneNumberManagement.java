 ...
 * delete) plus a {@code search} helper. Python's {@code purchase} / {@code release} convenience
 * aliases are folded into {@code create} (body includes the number to buy) and {@code delete}
 * (releases by ID). Search takes {@code Map<String,String>} — pass every value as a string.
 *
...
public class RestPhoneNumberManagement {
  ...
  public static void main(String[] args) {
          ...
          client
              ...
              .search(
                  Map.of(
                      "areacode", "512",
                      "max_results", "5"));
      // AvailablePhoneNumbersResponse is a pure-data DTO with no toString(); concatenating it
      // printed the
      // identity hash ("AvailablePhoneNumbersResponse@4488aabb") instead of any result. The numbers
      // live on `data`.
      System.out.println("  Available: " + results.data);
    } catch (RestError e) {
    ...
    try {
      var purchased = client.phoneNumbers().create(Map.of("number", "+15125551234"));
      String numberId = (String) purchased.get("id");
      System.out.println("  Purchased: " + numberId);

      // 4. Update the number (e.g. rename).
      System.out.println("\nUpdating phone number...");
      client.phoneNumbers().update(numberId, Map.of("name", "Main Office Line"));
