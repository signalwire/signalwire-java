...
The authoritative list of handler values. The Java SDK ships this as a typed enum so the value doesn't have to be guessed from the OpenAPI spec.
...
**Naming note on `PhoneCallHandler`:** It's deliberately *not* named `CallHandler` because the RELAY client already has a field called `onCallHandler` for inbound-call callbacks. The two live side-by-side without confusion.
...
The Java SDK deliberately does *not* expose `fabric.swmlWebhooks().create()` / `fabric.cxmlWebhooks().create()` on the Fabric namespace. If you find the underlying endpoints in an old curl script:

```
POST /api/fabric/resources/swml_webhooks          ← orphan; does nothing on its own
POST /api/fabric/resources/{id}/phone_routes       ← returns 404/422 for webhook types
```

…those calls leave orphans. The Fabric resource is materialized as a side-effect of `phoneNumbers().update`; there's nothing to attach.

### `assignPhoneRoute` is narrow and deprecated for the common case

Some older SDKs expose an `assignPhoneRoute` method. The Java SDK **does not**, by design. It applies only to a few legacy resource types that accept phone-route attachment as an explicit step (the authoritative list is in the server's OpenAPI spec). It **does not work** for `swml_webhook`, `cxml_webhook`, or `ai_agent` — those use the derivation model above.

## Example

See [`rest/examples/RestBindPhoneToSwmlWebhook.java`](../examples/RestBindPhoneToSwmlWebhook.java) for the canonical runnable recipe.
...
- `assignPhoneRoute` is narrow and not needed for the common handlers — the Java SDK does not surface it.
