...
//   java -cp build/scripts:build/libs/signalwire-sdk-4.0.0.jar:<runtime-deps> SignatureDump >
// raw_dump.json
//
...
public class SignatureDump {
  ...
  public static void main(String[] args) throws Exception {
    ...
    try (JarFile jar = new JarFile(jarPath)) {
      ...
      while (entries.hasMoreElements()) {
        ...
        if ("module-info.class".equals(n)) continue;
    ...
    for (String cls : classNames) {
      ...
      try {
        ...
        // A class whose optional compile-only dep (e.g. javax.servlet) is absent
        // from this classpath raises NoClassDefFoundError — a LinkageError, not an
        // Exception — so both arms are required. Narrower than Throwable, which
        // would also swallow OutOfMemoryError/StackOverflowError and turn a real
        // JVM failure into a silently short signature dump.
  ...
  /**
   * Recursively collect every public nested class within {@code outer}, skipping nested annotations
   * and the outer class itself.
   */
  static List<Class<?>> collectNestedPublic(Class<?> outer) {
    ...
    try {
      ...
      // best-effort: getDeclaredClasses() raises NoClassDefFoundError (a LinkageError)
      // when a nested type references an absent optional dep. Not Throwable — a JVM
  ...
  static String dumpType(Class<?> c) {
    ...
    String kind =
        c.isInterface()
            ? "interface"
            : c.isEnum() ? "enum" : c.isAnnotation() ? "annotation" : "class";
    ...
    Arrays.sort(
        methods, Comparator.comparing(Method::getName).thenComparing(SignatureDump::paramTypeSig));
    ...
    if (extendsReadResource(c) && !declaredMethodNames.contains("paginate")) {
      try {
        ...
        // best-effort: if paginate can't be resolved, skip. NoSuchMethodException is an
        // Exception; a paginate() whose signature references an absent optional dep
        // raises NoClassDefFoundError (a LinkageError). Not Throwable.
      }
    }
  ...
  /**
   * A stable, deterministic string of an executable's parameter types, used ONLY as a sort
   * tiebreaker so overloaded methods/constructors (same name / same param count) get a fixed order
   * independent of reflection's unspecified getDeclared*() ordering. Not emitted into the output —
   * purely for sorting. Uses Type.getTypeName() for the full (generic-aware) spelling.
   */
  ...
  static String dumpConstructor(Constructor<?> k) {
    ...
    sb.append("\"parameters\":")
        .append(dumpParams(k.getParameters(), k.getGenericParameterTypes()))
        .append(",");
  ...
  static String dumpMethod(Method m) {
    ...
    sb.append("\"parameters\":")
        .append(dumpParams(m.getParameters(), m.getGenericParameterTypes()))
        .append(",");
